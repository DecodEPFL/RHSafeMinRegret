...
source_file = 'init_rho0p7_hu2_hx3p5_hw1_T20_tol0p001_eps0p5_Hinf.mat';
if isfile(source_file)
    load(source_file);
else
    flag = 'Hinf';
    initialize_control_problem(); % Create sys, opt, and sls structures
    save init_rho0p7_hu2_hx3p5_hw1_T20_tol0p001_eps0p5_Hinf
end
...
for i = 1:size(w.profiles, 2)
    for j = 1:(ctrl.max_iter*w.is_stoch(i) + (1 - w.is_stoch(i)))
        ...
        for k = 1:size(ctrl.N, 2)
            ...
            hinf.cum_costs(i, j, k) = evaluate_receding_horizon_policy(sys, opt, sls, ctrl, k, 'Hinf', [], w.realization);
            hinf.cum_costs(i, j, k)
            fprintf('solved!\n\n');
        end
    end
end
